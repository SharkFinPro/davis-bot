module.exports = {
  ...
  command(message, bot) {
    ...
    message.channel.bulkDelete(parseInt(message.content.toLowerCase().split(" ")[1]) + 1, 10).then((messages) => {
      message.channel.send(`**Deleted ${messages.size - 1} messages**! 👍`).then((m) => m.delete({timeout: 2000}));
    });
  }
};
