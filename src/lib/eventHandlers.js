  ...
  async messageReactionAdd(messageReaction, user) {
    if (messageReaction.emoji.name === "⭐") {
      ...
      if (!starboardMessage && messageReaction.count >= config.starboardStars) {
        this.embedLog(messageReaction.message.guild.channels.find((channel) => channel.name === "starboard"), false, `${messageReaction.message.author.username}#${messageReaction.message.author.discriminator}`, messageReaction.message.author.avatarURL(), false, 0x00FF00, false, `${messageReaction.count}⭐ | ${messageReaction.message.id}`, [{name: "Channel", data: messageReaction.message.channel}, {name: "Message", data: `[${messageReaction.message.content !== "" ? messageReaction.message : "Jump To"}](${messageReaction.message.url})`}], messageReaction.message.attachments.first() ? messageReaction.message.attachments.first().url : "");
